class holtwinters:
    ...
    This class is to forecast timeseries on a Pandas Dataframe using Holt winters Forecasting model.
    ...
    def extract_actuals(self, pandasDF):
        '''
        author_contact: sulekha.aloorravi@gmail.com
        extract_actuals - Extract observed values from pandas dataframe for further processing
    ...
    def initiate_seasonal_indices_multiplicative(self,pandasDF,L):
        '''
        author_contact: sulekha.aloorravi@gmail.com
    ...
    def initiate_seasonal_indices_additive(self,pandasDF,L):
        '''
        author_contact: sulekha.aloorravi@gmail.com
    ...
    def holtwinters_multiplicative(self,pandasDF,alpha,beta,gamma,L,n_predictions):
        '''
        author_contact: sulekha.aloorravi@gmail.com
        ...
        observed = self.extract_actuals(pandasDF)
        period = int(len(observed)/L)
        SeasonalIndices,Avgs,Subset = self.initiate_seasonal_indices_multiplicative(pandasDF,L)
    ...
    def holtwinters_additive(self, pandasDF,alpha,beta,gamma,L,n_predictions):
        '''
        author_contact: sulekha.aloorravi@gmail.com
        ...
        observed = self.extract_actuals(pandasDF)
        period = int(len(observed)/L)
        SeasonalIndices,Avgs,Subset = self.initiate_seasonal_indices_additive(pandasDF,L)
        trendfactor = self.initiate_trend_factor(pandasDF,L)
        Seasonality = SeasonalIndices
        Predictions = []
        p = len(observed)
    ...
    def BestFit_Multiplicative(self,pandasDF, interval, denominator, L, n_predictions):
        '''
        author_contact: sulekha.aloorravi@gmail.com
        ...
        for i in [float(j) / denominator for j in range(0, denominator, interval)]:
            for a in [float(b) / denominator for b in range(0, denominator, interval)]:
                for l in [float(m) / denominator for m in range(0, denominator, interval)]:
                    Observed, Predictions, Level, Trend, Seasonality = self.holtwinters_multiplicative(pandasDF,i,a,l,L,n_predictions)
    ...
    def BestFit_Additive(self,pandasDF, interval, denominator, L, n_predictions):
        '''
        author_contact: sulekha.aloorravi@gmail.com
        ...
        for i in [float(j) / denominator for j in range(0, denominator, interval)]:
            for a in [float(b) / denominator for b in range(0, denominator, interval)]:
                for l in [float(m) / denominator for m in range(0, denominator, interval)]:
                    Observed, Predictions, Level, Trend, Seasonality = self.holtwinters_additive(pandasDF,i,a,l,L,n_predictions)
                    Acc.append(self.Accuracy(Observed, Predictions))
                    alpha.append(i)
                    beta.append(a)
                    gamma.append(l)
